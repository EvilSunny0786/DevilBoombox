...
class MongoDBClient(metaclass=Singleton):
    def __init__(self) -> None:
        ...
        self.theshashank = client["theshashank"]

    def fetchRunTimeData(self):
        try:
            return self.theshashank.runtime_data.find_one({'service': self.config.get('source')})
    ...
    def get_all_chats(self):
        try:
            data = self.theshashank.tgcalls_chats.find({})
    ...
    def add_tgcalls_chats(self, chatInfo):
        try:
            exists = self.theshashank.tgcalls_chats.find(
                {"chat_id": chatInfo['chat_id']})
            if list(exists) in [None, []]:
                self.theshashank.tgcalls_chats.insert_one(chatInfo)
    ...
    def add_song_playbacks(self, songInfo, requestedUser, docId):
        try:
            if self.config.get("env") == "prod":
                self.theshashank.tgcalls_playbacks.insert_one(
    ...
    def update_admins(self, chatId, admins):
        try:
            if isinstance(admins, list) is True:
                data = self.theshashank.tgcalls_chats.find_one_and_update(
    ...
    def add_tgcalls_users(self, userInfo):
        try:
            self.theshashank.tgcalls_users.replace_one(
    ...
    def chats_to_disconnect(self):
        try:
            ...
            result = self.theshashank['tgcalls_playbacks'].aggregate(agg)
            return list(result)
        except Exception as ex:
